...
\citep{Lawrence:1994, Lawrence:2000, Latouche:1992}.
...
@
\noindent
In order to evaluate goodness of fit, BIC and AIC are computed  \citep{mvtnorm:2014} both for the contamination model and for the simple Gaussian model ($\lambda=0$). This parameter should be used to prevent overfitting.
